...
public class Server {
    ...
    private static void readData(SelectionKey key, Selector selector) throws IOException {
        ...
        if (bytesRead == -1) {
            ...
            // Reverse the word using the reverseWord logic
            String reversedMessage = reverseWord(message);
            ...
            if (EXIT_COMMAND.equalsIgnoreCase(message.trim())) {
                ...
                ByteBuffer reversedBuffer = ByteBuffer.wrap(reversedMessage.getBytes());
                senderChannel.write(reversedBuffer);
            }
        }
    }
